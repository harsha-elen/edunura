...
import { getThumbnailUrl } from '@/services/courseService';
...
export default function Dashboard() {
    ...
    useEffect(() => {
        const fetchLiveClasses = async () => {
            try {
                const [enrollments, liveClassesRes] = await Promise.all([
                    enrollmentService.getEnrollments(),
                    apiClient.get('/live-classes/all'),
                ]);

                const enrolledCourseIds = (enrollments || []).map((e: EnrolledCourse) => e.course_id);
                ...
                if (liveClassesRes.data.status === 'success') {
                    ...
                    const transformed: LiveClass[] = filtered.slice(0, 3).map((cls: any) => {
                ...
                console.warn('Could not fetch live classes:', err);
            }
        };
    ...
    const stats = {
        coursesInProgress: enrolledCourses.filter((c) => c.status === 'active').length,
        assignmentsDue: 0,
        averageGrade: 0,
        studyHours: 0,
    };
                ...
                <Grid size={{ xs: 12, xl: 8 }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                        ...
                        <Link
                            ...
                            underline="hover"
                            sx={{ display: 'flex', alignItems: 'center', gap: 0.5, fontWeight: 600, fontSize: '0.875rem' }}
                        >
                                    ...
                                    <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 3 }}>
                                        ...
                                        >
                                            {course.thumbnail ? (
                                                <Box
                                                    component="img"
                                                    src={getThumbnailUrl(course.thumbnail) || undefined}
                                                    alt={course.title}
                                        ...
                                        <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', py: 0.5 }}>
                                            <Box>
                                                ...
                                                <Typography variant="body2" sx={{ color: 'text.secondary', mb: 2 }}>
                                                    {course.instructors || 'Instructor'} • {course.total_lessons || 0} Lessons
                                                </Typography>
                                            </Box>
                                            ...
                                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                                                <Box>
                                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                                                        ...
                                                        <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.primary' }}>{course.progress_percentage || 0}%</Typography>
                                                    </Box>
                                                    <LinearProgress
                                                        variant="determinate"
                                                        value={course.progress_percentage || 0}
                                                        color={getCourseColor(course.progress_percentage || 0) as any}
                                                ...
                                                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                    ...
                                                    />
                                                    <Button
                                                        variant={(course.progress_percentage || 0) > 0 ? 'contained' : 'outlined'}
                                                        color="primary"
                                                        size="small"
                                                        ...
                                                        sx={{
                                                            ...
                                                            px: 3,
                                                            fontWeight: 600,
                                                            bgcolor: (course.progress_percentage || 0) > 0 ? theme.palette.primary.main : 'transparent',
                                                            '&:hover': { bgcolor: (course.progress_percentage || 0) > 0 ? theme.palette.primary.dark : alpha(theme.palette.primary.main, 0.04) },
                                                        }}
                                                    >
                                                        {(course.progress_percentage || 0) > 0 ? 'Resume' : 'Start'}
                                                    </Button>
                                                </Box>
                                            </Box>
                                        </Box>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        ...
                        >
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                                ...
                                <IconButton size="small" sx={{ color: 'text.disabled' }} onClick={() => router.push('/student/live-classes')}>
                                    <CalendarIcon />
                                </IconButton>
                            </Box>
                                ...
                                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                                    {liveClasses.map((liveClass, i) => (
                                        <React.Fragment key={liveClass.id}>
                                            <Box
                                                onClick={() => router.push(liveClass.lesson ? `/course/${liveClass.course_id}/lesson/${liveClass.lesson.id}` : `/course/${liveClass.course_id}/learn`)}
                                                sx={{
                                                    display: 'flex',
                                                    gap: 2,
                                            ...
                                            >
                                                ...
                                                <Button
                                                    ...
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        router.push(liveClass.lesson ? `/course/${liveClass.course_id}/lesson/${liveClass.lesson.id}` : `/course/${liveClass.course_id}/learn`);
                                                    }}
                                                    sx={{
                                                        fontSize: '0.7rem',
                                                        fontWeight: 600,
