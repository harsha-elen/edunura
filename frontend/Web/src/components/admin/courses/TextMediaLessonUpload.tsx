        ...
        if (open) {
            ...
            if (draftRaw) {
                try {
                    ...
                    if (!initialData && typeof draft?.allowPreview === 'boolean') setAllowPreview(draft.allowPreview);
                } catch {
    ...
    useEffect(() => {
        ...
        const initializeEditor = async () => {
            try {
                ...
                // Dynamic imports for EditorJS (SSR-safe)
                const [
                    { default: EditorJS },
                    { default: Header },
                    { default: Paragraph },
                ...
                const editor = new EditorJS({
                    ...
                    tools: {
                        ...
                        code: { class: Code as any, inlineToolbar: false },
                        link: { class: LinkTool as any, inlineToolbar: false },
                        image: {
                            ...
                            config: {
                                uploader: {
                                    uploadByFile: async (file: File) => {
                                        ...
                                        try {
                                            const response = await uploadLessonResource(currentLessonId, file);
                        ...
                        embed: { class: Embed as any, inlineToolbar: false },
                        table: { class: Table as any, inlineToolbar: false },
                        marker: { class: Marker as any, inlineToolbar: true },
                        inlineCode: { class: InlineCode as any, inlineToolbar: true },
                        delimiter: { class: Delimiter as any, inlineToolbar: false },
                        raw: { class: Raw as any, inlineToolbar: false },
                    },
                    data: parsedData,
                    onChange: async () => {
    ...
    return (
        ...
        >
            <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                ...
                <Box sx={{ display: 'flex', flex: 1, overflow: 'hidden', bgcolor: theme.palette.mode === 'dark' ? '#0f172a' : '#f8fafc' }}>
                    {/* Main Content Area */}
                    <Box sx={{ flex: 1, overflowY: 'auto', p: 4, display: 'flex', flexDirection: 'column', gap: 4 }}>
                        ...
                        <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                            ...
                            <Paper sx={{
                                flex: 1, p: 3, borderRadius: '1rem',
                                border: `1px solid ${theme.palette.divider}`,
                                boxShadow: 'none', overflow: 'auto', minHeight: '400px',
                    ...
                    <Box sx={{ width: 360, borderLeft: `1px solid ${theme.palette.divider}`, bgcolor: theme.palette.background.paper, display: 'flex', flexDirection: 'column' }}>
                        <Box sx={{ p: 3, flex: 1, overflowY: 'auto' }}>
                            {/* Settings */}
                            <Box sx={{ mb: 4 }}>
                                <Typography variant="subtitle2" sx={{ fontWeight: 700, color: '#475569', mb: 2 }}>Lesson Settings</Typography>
                                <Box sx={{
                                    p: 2, borderRadius: '0.75rem',
                                    border: `1px solid ${theme.palette.divider}`,
                                    bgcolor: theme.palette.mode === 'dark' ? 'rgba(30,41,59,0.5)' : '#f8fafc',
                                    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                                }}>
                                    <Box>
                                        <Typography variant="body2" sx={{ fontWeight: 600 }}>Free Preview</Typography>
                                        <Typography variant="caption" sx={{ color: '#64748b' }}>Allow students to view without enrolling</Typography>
                                    </Box>
                                    <Switch checked={allowPreview} onChange={(e) => setAllowPreview(e.target.checked)} />
                                </Box>
                            </Box>
                            ...
                            <Box>
                                ...
                                <Box
                                    ...
                                    sx={{
                                        ...
                                        borderRadius: '0.75rem', p: 3, textAlign: 'center',
                                        bgcolor: theme.palette.mode === 'dark' ? 'rgba(30,41,59,0.5)' : '#f8fafc',
                                        cursor: 'pointer', mb: 3, transition: 'all 0.2s',
                                        '&:hover': { borderColor: theme.palette.primary.main, bgcolor: alpha(theme.palette.primary.main, 0.02) },
                                    }}
                                >
                                    <input id="admin-sidebar-resource-input" type="file" style={{ display: 'none' }} onChange={handleResourceFileChange} />
                                    <Box sx={{
                                        width: 40, height: 40, bgcolor: theme.palette.background.paper,
                                        borderRadius: '0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center',
                                        mx: 'auto', mb: 1.5, boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1)', border: `1px solid ${theme.palette.divider}`,
                                    }}>
                                        <AttachmentIcon sx={{ color: theme.palette.primary.main }} />
                                    </Box>
                                    <Typography sx={{ fontSize: '0.875rem', fontWeight: 600, mb: 0.5 }}>Click to upload or drag and drop resources</Typography>
                                    <Typography sx={{ fontSize: '0.75rem', color: '#64748b' }}>PDF, ZIP, DOCX (Max 50MB)</Typography>
                                </Box>

                                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                                    {existingResources.map((file, index) => (
                                        <Paper key={`existing-${index}`} sx={{
                                            p: 1.5, borderRadius: '0.75rem', border: `1px solid ${theme.palette.divider}`,
                                    ...
                                    {resourceFiles.map((file, index) => (
                                        <Paper key={`new-${index}`} sx={{
                                            p: 1.5, borderRadius: '0.75rem', border: '1px solid #bae6fd',
                                            display: 'flex', alignItems: 'center', gap: 2, boxShadow: 'none', bgcolor: '#f0f9ff',
                                            '&:hover': { bgcolor: '#e0f2fe' },
                                        }}>
                                            <Box sx={{
                                                width: 40, height: 40, borderRadius: '0.5rem', bgcolor: '#dbeafe',
                                                display: 'flex', alignItems: 'center', justifyContent: 'center',
                                            }}>
                                                <DescriptionOutlinedIcon sx={{ color: '#3b82f6', fontSize: 20 }} />
                                            </Box>
                            ...
                            <Box sx={{
                                mt: 4, p: 2, borderRadius: '0.75rem',
                                bgcolor: 'rgba(59, 130, 246, 0.05)', border: '1px solid rgba(59, 130, 246, 0.2)',
                                display: 'flex', gap: 2,
                            }}>
                ...
                <Box sx={{
                    ...
                    <Button onClick={handleCloseModal} sx={{ textTransform: 'none', fontWeight: 600, fontSize: '0.875rem', color: '#64748b', px: 3, py: 1.25, borderRadius: '0.5rem', '&:hover': { bgcolor: theme.palette.action.hover } }}>
                        Cancel
                    </Button>
                    <Button
                        ...
                        sx={{
                            ...
                            bgcolor: theme.palette.primary.main, px: 4, py: 1.25, borderRadius: '0.5rem', boxShadow: 'none',
