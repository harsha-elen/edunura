...
export const courseService = {
    getCourses: async (params?: { page?: number; limit?: number; status?: string; category?: string; search?: string }) => {
    ...
    uploadThumbnail: async (id: number, file: File, onProgress?: (progress: number) => void) => {
        const formData = new FormData();
        formData.append('thumbnail', file);
        const response = await apiClient.patch(`/courses/${id}/thumbnail?type=thumbnail`, formData, {
            headers: {
    ...
    uploadIntroVideo: async (id: number, file: File, onProgress?: (progress: number) => void) => {
        const formData = new FormData();
        formData.append('intro_video', file);
        const response = await apiClient.patch(`/courses/${id}/intro-video?type=preview`, formData, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
            onUploadProgress: (progressEvent) => {
