...
const Calendar: React.FC = () => {
    ...
    useEffect(() => {
        const fetchClasses = async () => {
            try {
                console.log('[CALENDAR] Fetching live classes...');
                const response = await getAllLiveClasses();
                console.log('[CALENDAR] API Response:', response);

                if (response.status === 'success') {
                    console.log('[CALENDAR] Setting live classes:', response.data);
                    setLiveClasses(response.data);
                } else {
                    console.warn('[CALENDAR] Unexpected response status:', response.status);
                }
            } catch (error) {
                console.error('[CALENDAR] Failed to fetch live classes:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            ...
            case 'upcoming': return theme.palette.warning.main; // Updated to Orange
    ...
    return (
        <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: '100%', bgcolor: theme.palette.background.paper }}>
            ...
            <Box sx={{ flex: 1, p: 3, display: 'flex', flexDirection: 'column', gap: 3 }}>
                            ...
                            <Box>
                                ...
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: '2px' }}>
                                    ...
                                    <Typography sx={{ fontSize: '14px', color: theme.palette.text.secondary }}>
                                        Starts at {new Date(nextSession.start_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                    </Typography>
                                </Box>
                            </Box>
                        </Box>
                ...
                {/* Legend and Info */}
                {/* Legend and Info with Navigation */}
                        ...
                        {Array.from({ length: calendarData.daysInCurrentMonth }).map((_, i) => {
                            ...
                            return (
                                <Box key={day} sx={{
                                    ...
                                    position: 'relative',
                                    ...(isToday && { bgcolor: alpha(theme.palette.primary.main, 0.04), ring: `1px inset ${alpha(theme.palette.primary.main, 0.1)}` }),
                                    '&:hover': { bgcolor: theme.palette.action.hover }
                                }}>
                                    <Box sx={{
                                        display: 'flex',
