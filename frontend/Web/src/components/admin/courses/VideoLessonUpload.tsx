...
const VideoLessonUpload: React.FC<VideoLessonUploadProps> = ({ open, onClose, onBack, onSave, uploadProgress, uploadError, initialData }) => {
    ...
    const handleVideoFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.files && event.target.files[0]) {
            ...
            if (file.size > maxSize) {
                setErrors({ ...errors, video: 'Video file must be under 500MB.' });
        ...
        if (videoSourceType === 'upload') {
            ...
            if (!videoUrl.trim()) {
                ...
                if (!youtubeRegex.test(videoUrl) && !vimeoRegex.test(videoUrl) && !genericUrlRegex.test(videoUrl)) {
                    newErrors.video = 'Please enter a valid video URL (YouTube, Vimeo, or direct link)';
                }
            }
        }
    ...
    return (
        <>
            ...
            >
                <style>{`
                    .ql-container * { max-width: 100% !important; word-wrap: break-word !important; overflow-wrap: break-word !important; box-sizing: border-box !important; }
                    .ql-editor p, .ql-editor h1, .ql-editor h2, .ql-editor h3, .ql-editor ol, .ql-editor ul, .ql-editor pre, .ql-editor blockquote { max-width: 100% !important; word-wrap: break-word !important; overflow-wrap: break-word !important; }
                `}</style>
                        ...
                        <Box>
                            ...
                            {uploadProgress !== undefined && !uploadError && (
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                    ...
                                    {uploadProgress < 100 && (
                                        <LinearProgress variant="determinate" value={uploadProgress} sx={{ width: 100, height: 6, borderRadius: 3 }} />
                                    )}
                                </Box>
                            )}
                            {uploadError && (
                                <Typography variant="caption" sx={{ color: '#dc2626', fontWeight: 700 }}>UPLOAD FAILED ✗</Typography>
                            )}
                        </Box>
                    </Box>
                    <IconButton
                    ...
                    <Box sx={{
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4, minWidth: 0, overflow: 'hidden' }}>
                                ...
                                <TextField
                                    ...
                                    sx={{
                                        '& .MuiOutlinedInput-root': {
                                            borderRadius: '0.5rem',
                                            '& fieldset': { borderColor: errors.title ? theme.palette.error.main : theme.palette.divider },
                                        }
                                    }}
                                />
                            </Box>
                            ...
                            <Box>
                                ...
                                <Box sx={{
                                    ...
                                    '& .ql-stroke': { stroke: '#64748b' },
                                    '& .ql-fill': { fill: '#64748b' },
                                    '& .ql-picker-label': { color: '#64748b' },
                                ...
                                <Box sx={{ mt: 2, display: 'flex', flexDirection: 'column', gap: 1 }}>
                                    {existingResources.map((file, index) => (
                                        <Box key={`existing-${index}`} sx={{
                                            ...
                                            p: 1.5, bgcolor: theme.palette.background.paper,
                                            border: `1px solid ${theme.palette.divider}`, borderRadius: '0.5rem',
                                        }}>
                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                                                <DescriptionIcon sx={{ color: '#3b82f6' }} />
                                                <Box>
                                                    <Typography sx={{ fontSize: '0.75rem', fontWeight: 600, color: theme.palette.text.primary }}>{file.name}</Typography>
                                                    <Typography sx={{ fontSize: '0.625rem', color: '#64748b' }}>{file.size} • Existing</Typography>
                                                </Box>
                                            </Box>
                                            <IconButton size="small" onClick={() => handleRemoveExisting(index)} sx={{ color: '#94a3b8', '&:hover': { color: '#ef4444' } }}>
                                    ...
                                    {resourceFiles.map((file, index) => (
                                        <Box key={`new-${index}`} sx={{
                                            ...
                                            <IconButton size="small" onClick={() => handleRemoveByFile(index)} sx={{ color: '#94a3b8', '&:hover': { color: '#ef4444' } }}>
                                                <CloseIcon sx={{ fontSize: '1.125rem' }} />
                                            </IconButton>
                                        </Box>
                                    ))}
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                            ...
                            <Box>
                                    ...
                                    >
                                        ...
                                        {uploadProgress !== undefined && uploadProgress < 100 ? (
                                            <Box sx={{ py: 2 }}>
                                                ...
                                                <Typography variant="caption" sx={{ color: '#64748b' }}>{uploadProgress}% complete</Typography>
                                            </Box>
                                        ) : videoFile ? (
                                            <Box sx={{ width: '100%', borderRadius: '0.5rem', overflow: 'hidden', bgcolor: '#000' }}>
                                                ...
                                                <Box sx={{ mt: 1, p: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
                                                    ...
                                                    <Button size="small" color="error" onClick={(e) => { e.stopPropagation(); setVideoFile(null); }}>
                                                        Remove
                                                    </Button>
                                                </Box>
                                            </Box>
                                            ...
                                            <Box sx={{ width: '100%', borderRadius: '0.5rem', overflow: 'hidden', bgcolor: '#000' }}>
                                                ...
                                                <Box sx={{ mt: 1, p: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between', bgcolor: theme.palette.mode === 'dark' ? '#1e293b' : '#f8fafc', borderRadius: '0.5rem' }}>
                                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                        <MovieEditIcon sx={{ color: theme.palette.primary.main, fontSize: '1.25rem' }} />
                                                        <Typography sx={{ fontSize: '0.875rem', fontWeight: 500, color: theme.palette.text.primary }}>Video uploaded</Typography>
                                                    </Box>
                                                    <Box sx={{ display: 'flex', gap: 1 }}>
                                                        {previousVideoPath && (
                                                            <Button size="small" onClick={(e) => { e.stopPropagation(); setUploadedVideoPath(previousVideoPath); setPreviousVideoPath(null); }} sx={{ textTransform: 'none', fontSize: '0.75rem' }}>
                                                                Undo
                                                            </Button>
                                                        )}
                                                        <Button size="small" variant="outlined" onClick={(e) => { e.stopPropagation(); setPreviousVideoPath(uploadedVideoPath); setUploadedVideoPath(null); document.getElementById('video-upload-input')?.click(); }} sx={{ textTransform: 'none' }}>
                                                            Replace
                                                        </Button>
                                                    </Box>
                                                </Box>
                                            ...
                                            <>
                                                <Box sx={{
                                                    width: 48, height: 48, bgcolor: theme.palette.background.paper,
                                                ...
                                                <Typography sx={{ fontSize: '0.875rem', fontWeight: 500, mb: 0.5, color: theme.palette.text.primary }}>Upload video file</Typography>
                                ...
                                {errors.video && (
                                    <Typography sx={{ color: theme.palette.error.main, fontSize: '0.75rem', mt: 1, ml: 0.5 }}>{errors.video}</Typography>
                                )}
                                {uploadError && (
                                    <Box sx={{ mt: 1, p: 1.5, bgcolor: '#fef2f2', border: '1px solid #fecaca', borderRadius: '0.5rem', display: 'flex', alignItems: 'center', gap: 1 }}>
                                        <Typography sx={{ color: '#dc2626', fontSize: '0.75rem', fontWeight: 500 }}>Upload failed: {uploadError}</Typography>
                                    </Box>
                                )}
                            </Box>
                            ...
                            <Box>
                                ...
                                <TextField
                                    fullWidth type="number" value={duration}
                                        ...
                                        input: {
                                            endAdornment: (
                                                <InputAdornment position="end">
                                                    <Typography sx={{ fontSize: '0.75rem', fontWeight: 500, color: '#94a3b8' }}>MIN</Typography>
                                                </InputAdornment>
                                            ),
                                        }
                                    }}
                            ...
                            <Box sx={{
                                ...
                                <Box>
                                    <Typography sx={{ fontSize: '0.875rem', fontWeight: 600, mb: 0.25, color: theme.palette.text.primary }}>Allow Preview</Typography>
                                    <Typography sx={{ fontSize: '0.75rem', color: '#64748b' }}>Watch without enrolling</Typography>
                                </Box>
                ...
                <Box sx={{
                    ...
                    <Button onClick={handleClose} sx={{ textTransform: 'none', fontWeight: 600, fontSize: '0.875rem', color: '#64748b', px: 3, py: 1.25, borderRadius: '0.5rem', '&:hover': { bgcolor: theme.palette.action.hover } }}>
                        Cancel
                    </Button>
                    <Button
                        ...
                        sx={{
                            ...
                            px: 3, borderRadius: '0.5rem', boxShadow: 'none',
                            '&:hover': { bgcolor: uploadProgress === 100 ? '#059669' : theme.palette.primary.dark, boxShadow: 'none' },
                        }}
                    >
                        {uploadProgress !== undefined && uploadProgress < 100
                            ? 'Uploading...'
            ...
            <Dialog open={uploadCancelDialog.open} onClose={() => setUploadCancelDialog({ open: false, action: null })} maxWidth="xs" fullWidth>
                ...
                <DialogActions sx={{ px: 3, pb: 2 }}>
                    <Button onClick={() => setUploadCancelDialog({ open: false, action: null })} sx={{ textTransform: 'none', color: '#64748b' }}>Continue Upload</Button>
                    <Button onClick={confirmCancelUpload} variant="contained" color="error" sx={{ textTransform: 'none' }}>Cancel Upload</Button>
                </DialogActions>
            </Dialog>
