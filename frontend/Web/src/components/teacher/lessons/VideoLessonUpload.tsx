...
export interface VideoLessonUploadProps {
    open: boolean;
    onClose: () => void;
    onBack?: () => void;
    onSave: (lessonData: {
...
const VideoLessonUpload: React.FC<VideoLessonUploadProps> = ({
    open,
    onClose,
    onBack,
    onSave,
    ...
    const [existingResources, setExistingResources] = useState<UploadedFile[]>((initialData?.resources || []) as UploadedFile[]);
    ...
    React.useEffect(() => {
        if (open && initialData) {
            ...
            setTitle(''); setDescription(''); setVideoUrl(''); setVideoFile(null);
            setVideoSourceType('upload'); setDuration(''); setAllowPreview(false);
            setUploadedVideoPath(null); setPreviousVideoPath(null);
            setExistingResources([]); setResourceFiles([]); setResourcesToDelete([]); setErrors({});
        }
    ...
    const handleVideoFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.files && event.target.files[0]) {
            ...
            if (file.size > 500 * 1024 * 1024) {
                ...
                if (previousVideoPath) { setUploadedVideoPath(previousVideoPath); setPreviousVideoPath(null); }
                return;
            }
    ...
    const handleRemoveExisting = (index: number) => {
        ...
        if (resource.id !== undefined) setResourcesToDelete([...resourcesToDelete, resource.id]);
    ...
    const handleSubmit = () => {
        ...
        if (videoSourceType === 'upload') {
            ...
            if (!videoUrl.trim()) newErrors.video = 'Please enter a valid video URL';
        }
        ...
        if (uploadProgress === undefined || uploadProgress >= 100) performClose();
    };

    const performClose = () => {
        setTitle(''); setErrors({}); setDescription(''); setVideoUrl('');
        setVideoFile(null); setVideoSourceType('upload'); setDuration('');
        setPreviousVideoPath(null);
        onClose();
    };
    ...
    const handleClose = () => {
        if (uploadProgress !== undefined && uploadProgress > 0 && uploadProgress < 100) {
            setUploadCancelDialog({ open: true, action: 'close' }); return;
        }
        performClose();
    };
    ...
    const handleBackClick = () => {
        if (uploadProgress !== undefined && uploadProgress > 0 && uploadProgress < 100) {
            setUploadCancelDialog({ open: true, action: 'back' }); return;
        }
        performBack();
    };
    ...
    const performBack = () => {
        ...
        setVideoFile(null); setVideoSourceType('upload'); setDuration('');
        setPreviousVideoPath(null);
        if (onBack) onBack(); else handleClose();
    };
    ...
    return (
        <>
            ...
            >
                    ...
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '6fr 4fr' }, gap: 5, width: '100%' }}>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                            ...
                            <Box>
                                ...
                                <Box
                                    onClick={() => document.getElementById('video-resource-file-input')?.click()}
                                    sx={{ border: '2px dashed #e2e8f0', borderRadius: '0.75rem', p: 3, textAlign: 'center', bgcolor: '#f8fafc', cursor: 'pointer', '&:hover': { borderColor: theme.palette.primary.main } }}
                                >
                                    <input id="video-resource-file-input" type="file" style={{ display: 'none' }} onChange={handleResourceFileChange} />
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                            ...
                            <Box>
                                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                                    ...
                                    <Box sx={{ display: 'flex', gap: 3 }}>
                                        {(['upload', 'url'] as const).map((type) => (
                                            <Box key={type} onClick={() => uploadProgress === undefined && setVideoSourceType(type)} sx={{ display: 'flex', alignItems: 'center', gap: 1, cursor: 'pointer', color: videoSourceType === type ? theme.palette.primary.main : '#64748b' }}>
                                                <Box sx={{ width: 18, height: 18, borderRadius: '50%', border: videoSourceType === type ? `5px solid ${theme.palette.primary.main}` : '2px solid #cbd5e1' }} />
                                                <Typography variant="caption" sx={{ fontWeight: videoSourceType === type ? 600 : 400 }}>
                                                    {type === 'upload' ? 'Upload Video' : 'External URL'}
                                                </Typography>
                                            </Box>
                                        ))}
                                    </Box>
                                ...
                                {videoSourceType === 'upload' ? (
                                    <Box
                                        sx={{ border: '2px dashed #e2e8f0', borderRadius: '0.75rem', p: (videoFile || uploadProgress !== undefined) ? 2 : 4, textAlign: 'center', bgcolor: '#f8fafc', cursor: (videoFile || uploadProgress !== undefined) ? 'default' : 'pointer', position: 'relative', ...(errors.video ? { borderColor: '#ef4444' } : {}) }}
                                        onClick={() => uploadProgress === undefined && document.getElementById('video-upload-input')?.click()}
                                    >
                                    ...
                                    <Box>
                                        <TextField
                                            ...
                                            InputProps={{ startAdornment: <InputAdornment position="start"><LinkIcon sx={{ fontSize: '1.125rem', color: '#94a3b8' }} /></InputAdornment> }}
                                        />
                                        {videoUrl.trim() && (
                                ...
                                {errors.video && videoSourceType === 'upload' && (
                                    <Typography variant="caption" sx={{ color: '#ef4444', mt: 0.5, display: 'block' }}>{errors.video}</Typography>
                                )}
                            </Box>
                            ...
                            <Box>
                                ...
                                <TextField
                                    ...
                                    placeholder="0"
                                    size="small"
                                    InputProps={{ endAdornment: <InputAdornment position="end"><Typography sx={{ fontSize: '0.75rem', fontWeight: 500, color: '#94a3b8' }}>MIN</Typography></InputAdornment> }}
                                />
                            </Box>
                        </Box>
                    </Box>
                ...
                <Box sx={{ px: 4, py: 2.5, bgcolor: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 2, borderTop: '1px solid #e2e8f0' }}>
                    ...
                    <Button
                        ...
                        sx={{ textTransform: 'none', bgcolor: uploadProgress === 100 ? '#10b981' : theme.palette.primary.main }}
                    >
            ...
            <Dialog open={uploadCancelDialog.open} onClose={() => setUploadCancelDialog({ open: false, action: null })} maxWidth="xs" fullWidth>
                ...
                <DialogActions>
                    <Button onClick={() => setUploadCancelDialog({ open: false, action: null })}>Continue Upload</Button>
                    <Button onClick={confirmCancelUpload} variant="contained" color="error">Cancel Upload</Button>
                </DialogActions>
            </Dialog>
