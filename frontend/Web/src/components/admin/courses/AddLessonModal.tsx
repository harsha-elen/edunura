...
const AddLessonModal: React.FC<AddLessonModalProps> = ({ open, onClose, onAdd, uploadProgress, initialData, courseId, sectionId }) => {
    ...
    const handleBackFromVideo = () => { setIsVideoUploadOpen(false); setShowLessonTypeSelector(true); };
    ...
    return (
        <>
            <Dialog
                ...
                maxWidth="sm"
                fullWidth
                PaperProps={{ sx: { borderRadius: '0.75rem', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', border: '1px solid #e2e8f0' } }}
            >
                {/* Header */}
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 3, py: 2.5, borderBottom: '1px solid #e2e8f0' }}>
                    <Box>
                        <Typography variant="h6" sx={{ fontWeight: 600, color: '#1e293b', fontSize: '1.125rem' }}>Add New Lesson</Typography>
                        <Typography variant="caption" sx={{ color: '#64748b', mt: 0.5, display: 'block', fontSize: '0.875rem' }}>Select the type of content you want to create for this module.</Typography>
                    </Box>
                    <IconButton size="small" onClick={handleClose} sx={{ color: '#94a3b8', '&:hover': { color: '#475569', bgcolor: 'rgba(226, 232, 240, 0.5)' } }}>
                    ...
                    {lessonTypes.map((type) => {
                        ...
                        return (
                            <Paper
                                ...
                                sx={{
                                    display: 'flex', alignItems: 'flex-start', gap: 2, p: 2, m: 0,
                                    border: 'none', borderRadius: '0.75rem', cursor: 'pointer',
                                    transition: 'all 0.2s ease-in-out', backgroundColor: '#ffffff', boxShadow: 'none',
                                    '&:hover': { backgroundColor: type.bgColor },
                                }}
                            >
                                <Box sx={{ flexShrink: 0, width: 48, height: 48, borderRadius: '0.5rem', backgroundColor: type.bgColor, display: 'flex', alignItems: 'center', justifyContent: 'center', color: type.iconColor }}>
                                    <Icon sx={{ fontSize: 24 }} />
                                </Box>
                                <Box sx={{ flex: 1 }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                        <Typography variant="body2" sx={{ fontWeight: 600, color: '#1e293b', fontSize: '0.9375rem' }}>{type.title}</Typography>
                                        <ChevronRightIcon sx={{ color: '#cbd5e1', fontSize: 20, ml: 2, flexShrink: 0 }} />
                                    </Box>
                                    <Typography variant="caption" sx={{ color: '#64748b', mt: 0.5, display: 'block', fontSize: '0.8125rem', lineHeight: 1.4 }}>{type.description}</Typography>
                                </Box>
                            </Paper>
                        );
                    })}
                ...
                {/* Footer */}
                <DialogActions sx={{ px: 3, py: 2, backgroundColor: '#f8fafc', borderTop: '1px solid #e2e8f0', gap: 1.5 }}>
                    <Button onClick={handleClose} sx={{ textTransform: 'none', fontWeight: 500, fontSize: '0.875rem', color: '#64748b', px: 1.5, py: 0.75, '&:hover': { backgroundColor: '#e2e8f0' } }}>
                        Cancel
                    </Button>
                </DialogActions>
            </Dialog>
            ...
            <VideoLessonUpload
                ...
                initialData={initialData && initialData.type === 'video' ? (() => {
                    ...
                    try { parsedMeta = initialData.meta ? JSON.parse(initialData.meta) : {}; } catch { parsedMeta = {}; }
                    ...
                    return {
                        ...
                        videoType: isExternalUrl ? 'url' as const : 'upload' as const,
                        videoUrl: resolvedVideoUrl,
                        videoPath: isUploadedFile ? initialData.file_path : undefined,
                        duration: initialData.duration?.toString() || '',
                        allowPreview: allowPreviewValue,
                        resources: (initialData.resources || []).map(r => ({ id: r.id, name: r.title, size: r.file_size, type: r.file_type })),
                    };
                })() : undefined}
            />
            ...
            <TextMediaLessonUpload
                ...
                initialData={initialData && initialData.type === 'text' ? (() => {
                    ...
                    try {
                        ...
                        const resolvedResources = (meta.resources && meta.resources.length > 0) ? meta.resources : mappedResources;
                        return { lessonId: initialData.id, title: initialData.title, description: resolvedDescription, allowPreview: meta.allowPreview || false, resources: resolvedResources };
                    } catch {
                        return { lessonId: initialData.id, title: initialData.title, description: initialData.meta || '', allowPreview: initialData.is_free_preview || false, resources: mappedResources };
                    }
                })() : undefined}
            />
            ...
            <SimpleLessonModal
                ...
                onClose={() => { setIsSimpleModalOpen(false); setShowLessonTypeSelector(true); onClose(); }}
